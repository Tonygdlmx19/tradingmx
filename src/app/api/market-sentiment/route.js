...
export async function GET() {
  try {
    ...
    } catch (_) { /* CNN endpoint failed, continue */ }
    ...
    if (avKey) {
      try {
        const vixRes = await fetch(
          `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=VIX&apikey=${avKey}`,
          { next: { revalidate: 600 } }
        );
        if (vixRes.ok) {
      ...
      } catch (_) { /* AV failed, try Yahoo as fallback */ }

      // Fallback: Yahoo Finance for VIX if Alpha Vantage failed
      if (!results.vix) {
        try {
